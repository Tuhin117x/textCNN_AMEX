...
path="E:\Storage Access\Workspaces\AMEX\Round_1\Train\input"
os.chdir(path)
...
#print(basedf)
#basedf.to_csv('train.csv',index=False)			

path2="E:\Storage Access\Workspaces\AMEX\Round_1\Train\output"
os.chdir(path2)
#print(os.listdir())


basedf2=pd.DataFrame(columns=['label','id','imagename'])
...
for file in os.listdir():
	if file.endswith(".json"):
		...
		with open(name) as f:
			...
			df=pd.read_json(name)
			#print(df)
			displaydf=wrangledata2(df,name)
			#print(displaydf)
			basedf2=pd.concat([basedf2,displaydf],axis=0)
			#print(basedf)

#print(basedf2)
#basedf2.to_csv('train_labels.csv',index=False)

final_df=basedf.merge(basedf2,on=['imagename','id'],how='left')
path3="E:\Storage Access\Workspaces\AMEX\Round_1"
os.chdir(path3)

final_df.to_csv('full_train.csv',index=False)